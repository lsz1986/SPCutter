...
BOOL CButtonST::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	// TODO: 在此添加专用代码和/或调用基类
	int MsgID = this->GetDlgCtrlID();
	switch (message)
	{
	case WM_LBUTTONDOWN:
		::SendMessage(this->GetParent()->m_hWnd, 9999, MsgID, 1);
		break;
	case WM_LBUTTONUP:
		::SendMessage(this->GetParent()->m_hWnd, 9999, MsgID, 0);
		break;
	default:
		break;
	}
